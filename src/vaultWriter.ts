...
export function extractDouyinLink(text: string): string | null {
  return extractDouyinLinks(text)[0] ?? null;
}

export function extractDouyinLinks(text: string): string[] {
  const pattern = new RegExp(DOUYIN_LINK_RE.source, "gi");
...
export async function writeNoteFromExtract(
  ...
  if (isVideo && !videoOnly) {
    fmLines.push(`transcript_source: ${data.transcript_source || "unknown"}`);
    if (data.transcript_source === "asr") {
      fmLines.push(`whisper_model: ${settings.whisperModel}`);
    }
  } else if (!isVideo && data.transcript_source) {
    fmLines.push(`transcript_source: ${data.transcript_source}`);
  }
  if (videoOnly) {
